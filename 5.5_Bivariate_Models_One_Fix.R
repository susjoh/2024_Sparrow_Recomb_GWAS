...
for(i in 1:nrow(modeltab)){
  ...
  eval(parse(text = paste0("model1 <- asreml(fixed  = cbind(", x1, "_m, ", x1, "_f) ~ trait + trait:Total_Coverage + trait:Total_Coverage2,
                     random = ~ corgh(trait, init = myinit.1):vm(parent, ", ifelse(modeltab$GRM[i] == "Pedigree", "ainv", "grminv"),") + idh(trait):ide(parent),
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
save.image("C:/Users/Susan/Desktop/test.RData")

write.table(resfixed,  "results/2_Bivar_Fixed_Effects_one_fix.txt", row.names = F, sep = "\t", quote = F)
write.table(resrandom, "results/2_Bivar_Random_Effects_one_fix.txt", row.names = F, sep = "\t", quote = F)
write.table(resVp,     "results/2_Bivar_Info_one_fix.txt", row.names = F, sep = "\t", quote = F)
save(resvparam, file = "results/2_Bivar_Vparams_one_fix.RData")
