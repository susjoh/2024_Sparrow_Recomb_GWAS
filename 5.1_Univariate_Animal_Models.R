...
for(i in 1:nrow(modeltab)){
  ...
  x <- x[,c(as.character(modeltab$RespVar[i]), "parent", "sex", "Total_Coverage")]
  ...
  eval(parse(text = paste0("model1 <- asreml(fixed = ", modeltab$RespVar[i], " ~ Total_Coverage + Total_Coverage2,
                      random = ~ vm(parent, ", ifelse(modeltab$GRM[i] == "Pedigree", "ainv", "grminv"),") + ide(parent),
                      data = x,
  ...
  meansq <- mean(x[,as.character(modeltab$RespVar[i])], na.rm = T)^2
  print(i)
  print((vpredict(model1, IA ~ V1/meansq)))
  ...
  resblup   <- rbind(resblup, xblup)
  

  rm(model1, xfixed, xrandom, xpin, xVp, x, xwald, xblup)
...
# resfixed <- read.table("results/5_Univar_Fixed_Effects.txt", header = T, sep = "\t", stringsAsFactors = F)
# resrandom <- read.table("results/5_Univar_Random_Effects.txt", header = T, sep = "\t", stringsAsFactors = F)
# respin <- read.table("results/5_Univar_Prop_of_Variance.txt", header = T, sep = "\t", stringsAsFactors = F)
# resVp <- read.table("results/5_Univar_Vp_and_Info.txt", header = T, sep = "\t", stringsAsFactors = F)
# reswald <- read.table("results/5_Univar_Wald_Fixed_Effects.txt", header = T, sep = "\t", stringsAsFactors = F)
# 
# names(resrandom)[8] <- "%ch"
# respin$ModelName <- NULL
...
for(i in 1:nrow(modeltab)){
  x <- subset(recsumm, sex == modeltab$Sex[i])
  x$Total_Coverage2 <- x$Total_Coverage^2
  x <- x[,c(as.character(modeltab$RespVar[i]), "parent", "sex", "Total_Coverage", "Total_Coverage2", "yapp_CO_count_no_micro")]
...
write.table(resfixed,  "results/5_Univar_Fixed_Effects_intracorr.txt", row.names = F, sep = "\t", quote = F)
write.table(resrandom, "results/5_Univar_Random_Effects_intracorr.txt", row.names = F, sep = "\t", quote = F)
write.table(respin,    "results/5_Univar_Prop_of_Variance_intracorr.txt", row.names = F, sep = "\t", quote = F)
write.table(resVp,     "results/5_Univar_Vp_and_Info_intracorr.txt", row.names = F, sep = "\t", quote = F)
write.table(reswald,   "results/5_Univar_Wald_Fixed_Effects_intracorr.txt", row.names = F, sep = "\t", quote = F)
write.table(resblup,   "results/5_Univar_BLUPs_intracorr.txt", row.names = F, sep = "\t", quote = F)

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
# 2. Parse Animal Model Results          #
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#

write.table(resfixed,  "results/5_Univar_Fixed_Effects.txt", row.names = F, sep = "\t", quote = F)
write.table(resrandom, "results/5_Univar_Random_Effects.txt", row.names = F, sep = "\t", quote = F)
write.table(respin,    "results/5_Univar_Prop_of_Variance.txt", row.names = F, sep = "\t", quote = F)
write.table(resVp,     "results/5_Univar_Vp_and_Info.txt", row.names = F, sep = "\t", quote = F)
write.table(reswald,   "results/5_Univar_Wald_Fixed_Effects.txt", row.names = F, sep = "\t", quote = F)
write.table(resblup,   "results/5_Univar_BLUPs.txt", row.names = F, sep = "\t", quote = F)


#~~ Quick visualisation

respin$ModelName <- paste(respin$RespVar, respin$Sex, respin$GRM, sep = "_")
respin$Effect %>% unique %>% dput

respin$Effect <- factor(respin$Effect, levels = rev(c("vm(parent, ainv)", "vm(parent, grminv)", "ide(parent)", "units!units")))

ggplot(subset(respin, GRM == "GRM"), aes(ModelName, Estimate, fill = Effect)) +
