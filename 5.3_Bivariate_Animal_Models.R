...
for(i in 1:nrow(modeltab)){
  ...
  write.table(resfixed,  "results/2_Bivar_Fixed_Effects_temp.txt", row.names = F, sep = "\t", quote = F)
  write.table(resrandom, "results/2_Bivar_Random_Effects_temp.txt", row.names = F, sep = "\t", quote = F)
  write.table(resVp,     "results/2_Bivar_Info_temp.txt", row.names = F, sep = "\t", quote = F)
  
  #~~ Retrospectively added the conditional heritabilities
  
  rescondit <- rbind(rescondit,
                     cbind(x, Sex = "Male", vpredict(model1, Va_m ~ V2/(V2 + V4 + V7))),
                     cbind(x, Sex = "Female", vpredict(model1, Va_f ~ V3/(V3 + V5 + V9))))
  
  write.table(rescondit,  "results/2_Bivar_Condit_heritabilities_temp.txt", row.names = F, sep = "\t", quote = F)
...
write.table(resfixed,  "results/2_Bivar_Fixed_Effects.txt", row.names = F, sep = "\t", quote = F)
write.table(resrandom, "results/2_Bivar_Random_Effects.txt", row.names = F, sep = "\t", quote = F)
write.table(resVp,     "results/2_Bivar_Info.txt", row.names = F, sep = "\t", quote = F)
save(resvparam, file = "results/2_Bivar_Vparams.RData")
write.table(rescondit,  "results/2_Bivar_Condit_heritabilities.txt", row.names = F, sep = "\t", quote = F)
